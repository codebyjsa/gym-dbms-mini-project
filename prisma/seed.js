...
// We will NOT use process.env to avoid any overrides
const dbConfig = {
...
console.log(`Connecting to ${dbConfig.database} at ${dbConfig.host} as user ${dbConfig.user}...`);
...
async function main() {
  ...
  try {
    ...
    console.log("Current DB User:", res[0]['CURRENT_USER()']);
    await conn.release();
  } catch (err) {
    console.error("❌ Native driver failed:", err.message);
    process.exit(1);
  }
